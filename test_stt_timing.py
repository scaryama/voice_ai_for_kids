...
# 2. STT 엔진 초기화 (Whisper 모델 로드 - 1회만)
print("[2] Whisper 모델 로드 중...")
start_init = time.time()
stt = STTEngine(config)
init_time = time.time() - start_init
print(f"[OK] 모델 로드 완료 ({init_time:.2f}초)\n")

# 3. 순수 음성-텍스트 변환 시간 측정 (모델 로드 제외)
print("[3] 순수 음성-텍스트 변환 시간 측정 중...")
print("-" * 70)
...
print()
print("=" * 70)
print("시간 측정 (Timing) - Whisper 모델 로드 제외, 순수 변환만 측정")
print("=" * 70)
print(f"Whisper 모델 로드 시간: {init_time:.2f}초 (1회, 이후 제외)")
print(f"순수 음성-텍스트 변환 시간: {transcribe_time:.2f}초 ⭐")
print(f"\n음성 길이: {audio_duration:.2f}초")
print(f"변환 속도: {audio_duration / transcribe_time:.2f}x (실시간 기준)")
print(f"→ 실시간 음성의 {audio_duration / transcribe_time:.1%} 속도로 변환 완료")
print("=" * 70)
